...
// Input Format:
// ---------------
// Line-1: Four space separated integers, N, K, 8 5 8 3, F
...
public class Program2{
    public static void main(String[] args){
        ...
        while(i < n){
            mp.put(treasure[i],mp.getOrDefault(treasure[i],0)+1);
            if(i-j+1 == k){
                PriorityQueue<int[]> pq = new PriorityQueue<>((a,b)->Integer.compare(b[0],a[0]));
                for(Map.Entry<Integer,Integer> e: mp.entrySet()){
                    pq.add(new int[]{e.getValue()*(int)Math.pow(e.getKey(),mf),e.getKey()});
                }
                int count = 0;
                int scores = 0;
                while(!pq.isEmpty() && count!=x){
                    int[] temp = pq.poll();
                    scores += temp[1]*mp.get(temp[1]);
                    count++;
                }
                res.add(scores);
                mp.put(treasure[j],mp.get(treasure[j])-1);
                if(mp.get(treasure[j])==0){
                    mp.remove(treasure[j]);
                }
                j++;
