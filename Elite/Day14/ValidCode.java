...
public class ValidCode{
    public static boolean isValid(String s,String valid){
        int i = 0,  j = 0;
        int n1=s.length(),n2=valid.length();
        while(i<n1 && j<n2){
            if(Character.isDigit(valid.charAt(j))!=true){
                ...
                int num=0;
                if(valid.charAt(j)-'0'==0)return false;
                while(j<n2 && Character.isDigit(valid.charAt(j))){
                    num = (num*10)+(valid.charAt(j)-'0');
                    j++;
                }
                i += num;
    ...
    public static void main(String[] args){
        ...
        System.out.println(isValid(s,valid));
    }
}