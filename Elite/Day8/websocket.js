...
// -->
// <config>
//     <url value="ws://10.11.17.223:8081"></url>
// </config>
...
server.on('connection',(socket)=>{
    ...
    socket.on('message',(message)=>{
        ...
        const func = message.split(" ");
        if(func[0].toUpperCase()==="INSERT" && func.length===3){
            if(!isNaN(parseInt(func[2]))){
                data.push({ID:data.length+1,name:func[1],salary:parseInt(func[2])});
        ...
        else if(func[0].toUpperCase()==="RETRIEVE"){
            if(data.length===0){
                socket.send('No employees found.');
            }
            data.forEach(emp=>socket.send(`ID: ${emp.ID}, Name: ${emp.name}, Salary: ${emp.salary}`));
        }
        else{
